            ...
            <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('peranan') is-invalid @enderror" id="peranan" name="peranan" required>
                                ...
                                <option value="super_admin" {{ old('peranan') == 'super_admin' ? 'selected' : '' }}>Super Admin</option>
                                <option value="admin" {{ old('peranan') == 'admin' ? 'selected' : '' }}>Admin</option>
                                <option value="ppp" {{ old('peranan') == 'ppp' ? 'selected' : '' }}>PPP</option>
                                <option value="ppk" {{ old('peranan') == 'ppk' ? 'selected' : '' }}>PPK</option>
                                <option value="pyd" {{ old('peranan') == 'pyd' ? 'selected' : '' }}>PYD</option>
                            </select>
                            @error('peranan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('pyd_group_id') is-invalid @enderror" id="pyd_group_id" name="pyd_group_id">
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('pyd_group_id') == $group->id ? 'selected' : '' }}>
                                        {{ $group->nama_kumpulan }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('jawatan') is-invalid @enderror" 
                                   id="jawatan" name="jawatan" value="{{ old('jawatan') }}">
                            @error('jawatan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('gred') is-invalid @enderror" 
                                   id="gred" name="gred" value="{{ old('gred') }}">
                            @error('gred')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('kementerian_jabatan') is-invalid @enderror" 
                           id="kementerian_jabatan" name="kementerian_jabatan" value="{{ old('kementerian_jabatan') }}">
...
<script>
    document.getElementById('peranan').addEventListener('change', function() {
        const pydGroupField = document.getElementById('pyd_group_id').parentElement;
        if (this.value === 'pyd') {
            pydGroupField.style.display = 'block';
        } else {
            pydGroupField.style.display = 'none';
        }
    });
